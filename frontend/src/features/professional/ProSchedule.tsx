...
export const ProSchedule: React.FC = () => {
  const [viewMode, setViewMode] = useState<'calendar' | 'list'>('calendar');
  const events = professionalMockService.getSchedule();

  // Calendar dates mock setup
      ...
      <div className="bg-white border border-brandLight-border p-6 rounded-3xl flex flex-col sm:flex-row justify-between sm:items-center gap-4 shadow-sm">
        <div>
          <h2 className="text-lg font-black text-brandDark-black font-display flex items-center space-x-2">
            ...
            <span>Professional Work Schedule</span>
          </h2>
          <p className="text-xs text-slate-500 font-bold mt-0.5">Track design reviews, site visits, and project delivery deadlines.</p>
        </div>
      ...
      {viewMode === 'calendar' ? (
                    ...
                    {dayEvents.map((e) => (
                      <div
                        ...
                        className={`text-[8px] p-1 rounded font-bold truncate cursor-pointer ${
                          e.type === 'Site Visit' ? 'bg-orange-50 text-orange-600 border border-orange-200/50' : e.type === 'Deadline' ? 'bg-red-50 text-red-600 border border-red-200/50' : 'bg-blue-50 text-blue-600 border border-blue-200/50'
                        }`}
                      >
                        {e.title}
                      </div>
        ...
        <div className="bg-white border border-brandLight-border p-6 rounded-3xl shadow-sm space-y-4">
          <h3 className="text-xs font-black text-brandDark-black uppercase tracking-wider pb-2 border-b border-slate-100">
            Cronological Deadlines List
          </h3>

          <div className="space-y-3">
            {events.map((e) => (
              <div key={e.id} className="p-4 bg-slate-50 border border-slate-100 rounded-2xl flex flex-col sm:flex-row justify-between sm:items-center gap-4 hover:border-slate-200 transition-all">
                <div className="space-y-1.5 text-xs font-bold text-slate-500">
                  <div className="flex items-center space-x-2">
                    <span className={`text-[9px] px-2 py-0.5 rounded-lg border font-black uppercase font-mono ${
                      e.type === 'Site Visit' ? 'bg-orange-50 border-orange-200 text-orange-600' : e.type === 'Deadline' ? 'bg-red-50 border-red-200 text-red-600' : 'bg-blue-50 border-blue-200 text-blue-600'
                    }`}>
                      {e.type}
                    </span>
